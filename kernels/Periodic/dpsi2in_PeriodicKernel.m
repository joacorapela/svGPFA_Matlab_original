function dGin = dpsi2in_PeriodicKernel(prs,X1,mu,sigma,xxQuad,wwQuad)
...
dGin = permute(G,[1 2 3 5 4]) .* (-0.5 * permute(ddist,[1 2 3 5 4])/(lengthscale^2) .* RowMask ...
    + 0.5 * permute(ddist,[1 2 3 5 4])/(lengthscale^2) .* ColMask ...
    + permute(mudist,[1 2 3 5 4])./(lengthscale^2*permute(ss,[1 2 3 5 4])).* RowMask ...
    + permute(mudist,[1 2 3 5 4])./(lengthscale^2*permute(ss,[1 2 3 5 4])) .* ColMask);
