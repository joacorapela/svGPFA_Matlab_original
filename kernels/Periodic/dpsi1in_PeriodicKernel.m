function dPsi1in = dpsi1in_PeriodicKernel(prs,X1,mu,sigma,xxQuad,wwQuad)
...
GQuad = dKin_PeriodicKernel(prs,QuadEval,X1); % TQ x M x numhprs x R matrix

GQuad = permute(reshape(GQuad,T,Q,M,[],R),[1 2 4 5 3]);

dPsi1in = sum(bsxfun(@times, GQuad, permute(wwQuad,[2 3 4 5 1])),5);
