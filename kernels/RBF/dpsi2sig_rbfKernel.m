function dGsig = dpsi2sig_rbfKernel(prs,X1,mu,sigma,varargin)
...
if nargin < 5
    G = variance^4./sqrt(ss) .* exp(- (permute(ddist,[1 2 4 3])/(4*lengthscale^2) +  mudist.^2 ./(lengthscale^2*ss))); % N x N x T x M
else
    G = varargin{1};
end
ss2 = 2 *permute(sigma,[2 4 1 3]) + lengthscale^2;
dGsig = G .* (2 * mudist.^2./(ss2).^2 - 1 ./ (ss2));