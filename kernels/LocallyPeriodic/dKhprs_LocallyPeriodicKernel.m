...
% hyperparameters
variance = 1;
lengthscale_se = prs(1);
lengthscale_per = prs(2);
period_per = prs(3);
...
dKhprs(:,:,1,:) = G1.*G2/lengthscale_se^3 .* dd.^2;
dKhprs(:,:,2,:) = permute(4*G1.*G2/lengthscale_per^3 .*sin(rr).^2,[1 2 4 3]);
dKhprs(:,:,3,:) = permute(4*G1.*G2*pi.*dd/period_per^2.*sin(rr)/lengthscale_per^2 .*cos(rr),[1 2 4 3]);
