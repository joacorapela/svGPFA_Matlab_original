function [dGin2, dGin1] = dKin_cosineKernel(prs,X1,varargin)
...
dGin2(ColMask) = variance^2./lengthscale^2 .* sin(ddist./lengthscale^2);
...
if nargout > 1
    ...
    if nargin == 2 % grad wrt to first input if same
        dGin1(RowMask) = -variance^2./lengthscale^2 .* sin(ddist./lengthscale^2);
        dGin1(ColMask) = variance^2./lengthscale^2 .* sin(ddist./lengthscale^2);
